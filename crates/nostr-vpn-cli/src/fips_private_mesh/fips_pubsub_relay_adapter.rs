...
use nostr_pubsub_fips::{
    FipsPubsubClient, FipsPubsubClientOptions, FipsPubsubSubscription,
};
use nostr_pubsub_relay::RelayEventBus;
use nostr_sdk::prelude::{Event, Filter, Kind, RelayPoolNotification};
...
pub(crate) struct FipsPubsubNostrRelayAdapter {
    shutdown: Option<oneshot::Sender<()>>,
    task: JoinHandle<()>,
}

impl FipsPubsubNostrRelayAdapter {
    pub(crate) async fn start(endpoint: Arc<FipsEndpoint>, relays: &[String]) -> Result<Self> {
        let local_pubkey = PublicKey::parse(endpoint.npub())
            .context("invalid local FIPS endpoint identity")?;
...
mod fips_pubsub_relay_adapter_tests {
    ...
    use fips_core::config::{
        IdentityConfig, NostrRelayConfig, PeerConfig, TransportInstances,
    };
    use fips_core::{
    ...
    use nostr_vpn_core::fips_control::{
        FipsControlFrame, JoinRosterControl, NetworkRoster, SignedRoster,
    };
    ...
    async fn standard_join_and_roster_cross_the_fips_pubsub_relay_carrier() {
        ...
        let provider_identity =
            Identity::from_secret_bytes(&[12; 32]).expect("provider identity");
        ...
        let provider_client = FipsPubsubClient::start(
            Arc::clone(&provider),
            FipsPubsubClientOptions::default(),
        )
        .await
        ...
        let join_send = tokio::spawn(async move { guest_sender.send(admin_peer, &join_frame).await });
        ...
        let roster = SignedRoster::sign(
            ...
            NetworkRoster {
                ...
                devices: vec![admin_keys.public_key().to_hex(), guest_keys.public_key().to_hex()],
        ...
        assert!(matches!(received.frame, FipsControlFrame::JoinRoster { .. }));
        roster_send
            .await
    ...
    fn test_guest_config(
        ...
        config.transports.nostr_relay =
            TransportInstances::Single(NostrRelayConfig::default());
        config.peers = vec![

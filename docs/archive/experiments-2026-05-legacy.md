# Legacy Experiments - 2026-05

Archived from `docs/EXPERIMENTS.md` when the dataplane safety-net and ownership-refactor work became the current chronology. These notes remain useful for historical performance context, but they are not the current status of the FIPS/nvpn dataplane plan.

Durable takeaways preserved by the current safety-net and architecture plan:

- macOS sender behavior was directional and sensitive to UDP backpressure; real Mac Wi-Fi/screenshare soak is still operator-local.
- Darwin `sendmsg_x`, fixed sleeps, oversized queues, and blind batch-size changes did not provide a clean, durable fix.
- Connected UDP, bounded queues, explicit drop/backpressure counters, and small-control/priority classification remained useful.
- Direct-route, routed-FIPS, NAT, rekey, and MTU lessons are now covered by the safety-net, release/e2e gates, or current architecture invariants.
...
- Both nvpn peers used the LAN MTU profile with tunnel MTU 1290. The test used
  fixed per-peer tunnel `/32` routes.
...
- `./scripts/e2e-fips-nat-safe-mtu-docker.sh` passed with Bob using the Docker
  NAT route to Alice, Alice observing Bob through the NAT public address, and
  976-byte UDP payload files received on both sides.
...
- Generated Rust build artifacts were the main safe cleanup target, including
  about 20 GiB in `<home>/src/fips/target` and 6.4 GiB in
  `<home>/src/nostr-vpn/target`.

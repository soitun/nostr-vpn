...
| Network invite onboarding | `activeNetworkInvite` + `outboundJoinRequest` | Admin shares the network invite; importing it queues the normal FIPS join request | Required | Required | Required | Required | Required | Joining devices do not publish a separate approval QR or use an approval relay. |

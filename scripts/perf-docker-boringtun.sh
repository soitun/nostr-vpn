...
run_test_json() {
  ...
  case "$direction" in
    ...
    b_to_a)
      ...
      ;;
    *)
      printf 'perf-boringtun: invalid iperf direction for %s: %s\n' "$phase" "$direction" >&2
      return 2
      ;;
  esac
  [[ "${1:-}" == "-u" ]] && is_udp=1
...
run_boringtun_pass() {
  ...
  run_test_json tcp-single a_to_b "TCP single stream (A -> B)" "$tcp_single_json" "$cpu_phases"
  run_test_json tcp-single-b-to-a b_to_a "TCP single stream (B -> A)" "$tcp_single_reverse_json" "$cpu_phases"
  run_test_json tcp-4 a_to_b "TCP 4 streams (A -> B)" "$tcp_4_json" "$cpu_phases" -P 4
  run_test_json tcp-4-b-to-a b_to_a "TCP 4 streams (B -> A)" "$tcp_4_reverse_json" "$cpu_phases" -P 4
  run_test_json tcp-8 a_to_b "TCP 8 streams (A -> B)" "$tcp_8_json" "$cpu_phases" -P 8
  run_test_json tcp-8-b-to-a b_to_a "TCP 8 streams (B -> A)" "$tcp_8_reverse_json" "$cpu_phases" -P 8
  run_test_json udp-200 a_to_b "UDP 200 Mbit target (A -> B)" "$udp_200_json" "$cpu_phases" -u -b 200M
  run_test_json udp-200-b-to-a b_to_a "UDP 200 Mbit target (B -> A)" "$udp_200_reverse_json" "$cpu_phases" -u -b 200M
  ...
  run_test_json udp-1000 a_to_b "UDP 1000 Mbit target (A -> B)" "$udp_1000_json" "$cpu_phases" "${udp1000_args[@]}"
  run_test_json udp-1000-b-to-a b_to_a "UDP 1000 Mbit target (B -> A)" "$udp_1000_reverse_json" "$cpu_phases" "${udp1000_args[@]}"
...
main() {
  ...
  docker_bench_configure_iperf_socket_buffer_limits perf-boringtun "$IPERF_SOCKET_BUFFER"

...
assert_eq "$(jq -r '.pipeline.node_a_fips_hard_events' "$tmpdir/summary.json")" "encrypt_worker_bulk_dropped:max_rate_per_sec=5,total=10" "node-a FIPS hard events"
assert_eq "$(jq -r '.pipeline.node_a_nvpn_hard_events' "$tmpdir/summary.json")" "nvpn_tun_to_mesh_bulk_dropped:max_rate_per_sec=2,total=7" "node-a nvpn hard events"

...
cleanup() {
  ...
  if [[ "$COMPOSE_TOUCHED" == "1" && -z "${KEEP:-}" ]]; then
...
configure_iperf_socket_buffer_limits() {
  ...
  for service in node-a node-b; do
    ...
    if [[ ! "$actual_rmem" =~ ^[0-9]+$ ]] \
      ...
      echo "perf: failed to raise UDP socket buffer sysctls in $service for NVPN_DOCKER_IPERF_SOCKET_BUFFER=$IPERF_SOCKET_BUFFER (wanted >=$bytes, got rmem_max=${actual_rmem:-unknown}, wmem_max=${actual_wmem:-unknown})" >&2
